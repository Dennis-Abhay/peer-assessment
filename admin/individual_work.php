...
<html lang="en">
<head>
<title>Admin|Individual work</title>
...
<form class="form-horizontal col-sm-offset-2" role="form" method="post">
  <div class="form-group col-sm-10">
    <label class="control-label col-sm-3" for="name">Matric No:</label>
    <div class="col-sm-8">
      <select class="form-control"  id="name" name="user"required>
	  <?php 
		  ...
		  {
			  ?>
			  <option value="<?php echo $fetch_group['id'];?>"><?php echo $fetch_group['matric_no'];?></option>
		 <?php }
	  ?>
	  </select>
    </div>
...
{
  $sid=$_POST['user'];
  $query_report = "SELECT * FROM report INNER JOIN student ON report.student_id=student.id WHERE student_id='$sid'";
  ...
  <p></p>
  <label for='comment' class='col-sm-10'>Submitted By:".$row_report['name']."/".$row_report['matric_no']."<span class='col-sm-offset-6'>".$row_report['date']."</span></label>
