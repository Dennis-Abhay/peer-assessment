...
<html lang="en">
<head>
<title>Admin|Assign student</title>
...
<form class="form-horizontal col-sm-offset-2" role="form" method="post">
  <div class="form-group col-sm-10">
    ...
    <label class="control-label col-sm-3" for="name">Matric Number:</label>
    <div class="col-sm-8">
      <select class="form-control"  id="name" name="student" required>
	  <?php 
		  $qs="SELECT * FROM `student`";
		  $row_student=mysql_query($qs);
		  while($fetch_student=mysql_fetch_array($row_student))
		  {
			  ?>
			  <option value="<?php echo $fetch_student['id'];?>"><?php echo $fetch_student['matric_no'];?></option>
		 <?php }
	  ?>
	  </select>
    </div>
